...
<body>
    @php
        $selectedCalendarType = old('calendar_type', 'private');
        $selectedCalendarId = old('calendar_id', $calendarId ?? ($calendars->first()->id ?? ''));
        $baseDate = !empty($selectedDate) ? \Carbon\Carbon::parse($selectedDate) : now();

        $oldRepeatType = old('repeat_type', 'none');
        $oldRepeatEndType = old('repeat_end_type', 'never');
        $oldRepeatUntil = old('repeat_until', '');
        $oldRepeatCount = old('repeat_count', '');
        $oldRepeatMonthMode = old('repeat_month_mode', 'same_day');
        $oldRepeatMonthNth = old('repeat_month_nth', '1');
        $oldRepeatMonthWeekday = old('repeat_month_weekday', 'mon');
        $oldRepeatWeekdays = old('repeat_weekdays', []);

        if (!is_array($oldRepeatWeekdays)) {
            $oldRepeatWeekdays = [];
        }
    @endphp
    ...
    <div class="form-page">
        <div class="form-header">
            <a
                href="/calendar?year={{ $baseDate->year }}&month={{ $baseDate->month }}@if($selectedCalendarId)&calendar_id={{ $selectedCalendarId }}@endif"
                class="header-back"
            >
                カレンダーに戻る
            </a>
            <button type="submit" form="eventForm" class="header-save">保存</button>
        </div>

        <form method="POST" action="/events" class="event-form" id="eventForm">
            @csrf
    ...
    @enderror
                    ...
                    <div style="flex: 1;">
                        <div class="schedule-date-wrap @error('event_date') input-error @enderror" id="startDateWrap">
                            ...
                            <div class="schedule-date-display" id="startDateDisplay">
                                {{ old('event_date', $selectedDate ?? '') ?: '年 / 月 / 日' }}
                            </div>
                    ...
                    <div style="flex: 1;">
                        <div class="schedule-date-wrap @error('end_date') input-error @enderror" id="endDateWrap">
                            <input
                                ...
                                name="end_date"
                                class="schedule-date-native"
                                value="{{ old('end_date', $selectedDate ?? '') }}"
                                id="endDateInput"
                            >
                            <div class="schedule-date-display" id="endDateDisplay">
                                {{ old('end_date', $selectedDate ?? '') ?: '年 / 月 / 日' }}
                            </div>
                            <span class="schedule-icon">📅</span>
                        </div>
                        @error('end_date')
                ...
                <div class="custom-color-picker @error('color') input-error @enderror" style="border-radius: 14px;">
                    <button type="button" class="color-selected" id="colorToggle">
                        <span class="color-left">
                            <span class="color-circle color-green" id="selectedColorCircle"></span>
                            <span id="selectedColorText">エメラルド・グリーン</span>
                        </span>
                    ...
                    <input type="hidden" name="color" id="selectedColorValue" value="{{ old('color', 'green') }}">
                </div>
                @error('color')
                    <p class="field-error">{{ $message }}</p>
                @enderror
            ...
            <div class="form-group">
                ...
                <input type="hidden" name="repeat_type" id="repeat_type" value="{{ $oldRepeatType }}">
                <input type="hidden" name="repeat_end_type" id="repeat_end_type" value="{{ $oldRepeatEndType }}">
                <input type="hidden" name="repeat_until" id="repeat_until" value="{{ $oldRepeatUntil }}">
                <input type="hidden" name="repeat_count" id="repeat_count" value="{{ $oldRepeatCount }}">
                <input type="hidden" name="repeat_month_mode" id="repeat_month_mode" value="{{ $oldRepeatMonthMode }}">
                <input type="hidden" name="repeat_month_nth" id="repeat_month_nth" value="{{ $oldRepeatMonthNth }}">
                <input type="hidden" name="repeat_month_weekday" id="repeat_month_weekday" value="{{ $oldRepeatMonthWeekday }}">

                <div id="repeatWeekdaysHiddenWrap" style="display:none;">
                    @foreach($oldRepeatWeekdays as $weekday)
                        <input type="hidden" name="repeat_weekdays[]" value="{{ $weekday }}">
                    @endforeach
                </div>

                @error('repeat_weekdays')
                    <p class="field-error">{{ $message }}</p>
                @enderror
                @error('repeat_month_mode')
                    <p class="field-error">{{ $message }}</p>
                @enderror
                @error('repeat_month_nth')
                    <p class="field-error">{{ $message }}</p>
                @enderror
                @error('repeat_month_weekday')
                    <p class="field-error">{{ $message }}</p>
                @enderror
                @error('repeat_until')
                    <p class="field-error">{{ $message }}</p>
            ...
            <div class="form-group">
                ...
                <select id="notification_time" name="notification_time" class="@error('notification_time') input-error @enderror">
                    <option value="" {{ old('notification_time') == '' ? 'selected' : '' }}>通知しない</option>
                    <option value="5" {{ old('notification_time') == '5' ? 'selected' : '' }}>5分前</option>
                    <option value="10" {{ old('notification_time') == '10' ? 'selected' : '' }}>10分前</option>
                    <option value="30" {{ old('notification_time') == '30' ? 'selected' : '' }}>30分前</option>
                    <option value="60" {{ old('notification_time') == '60' ? 'selected' : '' }}>1時間前</option>
                    <option value="1440" {{ old('notification_time') == '1440' ? 'selected' : '' }}>1日前</option>
                </select>
